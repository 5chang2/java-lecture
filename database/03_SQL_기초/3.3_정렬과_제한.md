...
1. 모든 국가를 인구가 많은 순서대로 조회 (상위 10개)
2. 국가명을 알파벳순으로 조회 (상위 10개)
3. 평균 수명이 긴 순서대로 상위 10개 조회
...
"대륙별로 그룹화하고, 같은 대륙 내에서는 인구가 많은 순서대로 정렬" (상위 20개)
...
"아시아 국가 중에서 평균 수명이 70세 이상인 국가를 평균 수명이 긴 순서대로 상위 10개 조회"
...
city 테이블에서 다음을 조회하세요.

1. 대한민국(KOR)의 도시를 인구 순으로 조회
2. 전 세계 도시 중 인구 상위 20개 조회
3. 인구 100만~500만 사이의 도시를 인구 순으로 10개 조회
...
3. **실제 데이터**: world DB의 다양한 데이터로 정렬 체험
